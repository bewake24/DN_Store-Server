...
router.route("/login").post(limiter("30s", 2), loginUser); // if using form data in frontend then add upload.none() middleware in the route
//    or check:=> Content-Type multipart/form-data (in postman)
//    or send data in form data in postman as: x-www-form-urlencoded

//Secured Routes
router.route("/logout").post(limiter("30s", 1), verifyJWT, logout);
